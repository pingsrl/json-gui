...
describe('Interazioni con l\'albero', () => {
  ...
  it('expandAll espande tutti i nodi', async () => {
    ...
    // Aspetta completamento (expandAll è streaming)
    await browser.waitUntil(
      () => browser.execute(() => !window.__jsonStore.getState().loading),
      { timeout: 15000, timeoutMsg: 'expandAll non terminato' }
    );

    const state = await getStoreState(browser);
    // users(1) + settings(1) + tags(1) + i 3 user objects + i loro address objects = almeno 7
