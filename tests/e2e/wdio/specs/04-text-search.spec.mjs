...
describe('Ricerca testuale', () => {
  ...
  it('ricerca case-sensitive: "alice johnson" non trova "Alice Johnson"', async () => {
    await search(browser, 'alice johnson', { caseSensitive: true, target: 'values' });
    const count = await browser.execute(() =>
      window.__jsonStore.getState().searchResults.length
    );
    // Il nome è presente solo con iniziali maiuscole.
    expect(count).toBe(0);
  });
  ...
  it('trova "dark" nei values', async () => {
    await search(browser, 'dark', { target: 'values' });
    const count = await browser.execute(() =>
      window.__jsonStore.getState().searchResults.length
    );
    expect(count).toBeGreaterThan(0);
  ...
  it('digitare nel campo input avvia la ricerca', async () => {
    await browser.execute(() => {
      const input = document.getElementById('primary-search-input');
