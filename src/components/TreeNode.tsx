...
export const TreeNode: FC<Props> = ({ node, depth }) => {
  ...
  return (
    <div>
      <div
        ...
        className={`flex items-center gap-1 py-0.5 cursor-pointer select-none text-sm font-mono ${
          isSelected
            ...
            : isFocused
            ? 'outline outline-2 outline-yellow-400/70 bg-gray-700/50'
            : 'hover:bg-gray-700'
        }`}
      ...
      >
        <span className="w-4 text-gray-500 flex-shrink-0 flex items-center justify-center">
          {node.has_children ? (
            isExpanded ? <ChevronDown size={12} /> : <ChevronRight size={12} />
          ) : null}
        </span>
        {node.key !== null && (
          <span className="text-gray-300 flex-shrink-0">{node.key}:&nbsp;</span>
        )}
        <span className={`${TYPE_COLORS[node.value_type] ?? 'text-gray-300'} truncate`}>
          {node.value_preview}
        </span>
        {node.has_children && (
      ...
      {contextMenu && (
        <div
          ref={menuRef}
          className="fixed z-50 bg-gray-800 border border-gray-600 rounded shadow-lg py-1 text-sm text-gray-200 min-w-[160px]"
